...
values = [condition.value for condition in run.conditions]
```
...
[SQLAlchemy documentation](https://docs.sqlalchemy.org/en/20/orm/session_basics.html#basics-of-using-a-session)
...
When we've got query ready, we can actually get objects by `query.first()` or `query.all()`
...
Lets say, we want to get all runs with **event_count** > **100000**
...
query = db.session.query(Run).join(Run.conditions).join(Condition.type)\
        ...
        .filter(Condition.int_value > 100000)\
        .order_by(Run.number)
...
Lets look at the example, where we fill DB with dummy data and then query for runs using the helper properties. The same example can be found in `python/examples/90_advanced_sqlalchemy_query.py`
...
event_count_type = db.create_condition_type("event_count", ConditionType.INT_FIELD, "Number of events in the run")
data_value_type = db.create_condition_type("data_value", ConditionType.FLOAT_FIELD, "Some float data value")
...
[SQLAlchemy querying tutorial](https://docs.sqlalchemy.org/en/20/orm/tutorial.html#querying)
...
```bash
python python/examples/90_advanced_sqlalchemy_query.py
```
